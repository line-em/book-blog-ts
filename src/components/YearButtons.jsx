    ...
    <>
      <a
        ...
        className={`${hasFilter ? "" : "inactive pointer-events-none opacity-50"} 
			flex items-center px-2 py-2 text-zinc-700 rounded-xl shadow-sm bg-slate-50 gap-4 w-full self-center hover:text-teal-500`}
      >
      ...
      {validYears.map(curr => (
		...
		flex items-center p-1 sm:px-4 sm:py-2 text-zinc-700 rounded-xl sm:shadow-sm sm:bg-slate-50 gap-4 hover:text-teal-500`}
          href={`/${Number(curr)}`}
        >
          <BookOpenText
            size={24}
