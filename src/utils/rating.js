export const getStars = (rating) => {
    ...
    for (let i = rating; i >= 0.5; i--) {
        if (i <= 0.80) {
            output.push(
                `<img src="/assets/star-half-fill.svg" class="text-gradient"  alt="Half Star"/>`
                // `<i class="ph-star-half-fill ph-lg text-gradient" aria-hidden="true"></i>`
            );
        } else {
            output.push(
                `<img src="/assets/star-fill.svg" class="text-gradient"  alt="Full Star"/>`
    ...
    for (let i = 5 - rating; i >= 1; i--)
        ...
        output.push(`<img src="/assets/star.svg" class="text-gradient"  alt="Half Star"/>`);
